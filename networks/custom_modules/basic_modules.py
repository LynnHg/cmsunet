...
class EncoderBlock(nn.Module):
    def __init__(self, ch_in, ch_out, depth=2, use_dilation=False, use_res=False):
        ...
        if use_dilation:
            ...
            self.conv = [nn.Sequential(
                nn.Conv2d(ch_in, ch_out, kernel_size=3, padding=dilations[0], dilation=dilations[0], bias=False),
                nn.BatchNorm2d(ch_out),
                nn.ReLU(inplace=True),
            )]
            for i in range(1, depth):
                self.conv.append(nn.Sequential(nn.Conv2d(ch_out, ch_out, kernel_size=3, padding=dilations[0], dilation=dilations[0], bias=False),
                                               nn.BatchNorm2d(ch_out),
